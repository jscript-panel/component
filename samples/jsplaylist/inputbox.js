...
function oInputbox(w, h, default_text, empty_text, func) {
	...
	this.CalcText = function () {
		this.TWidth = this.text.substr(this.offset).calc_width2(g_font_12);
	}

	this.GetCx = function (pos) {
		if (pos >= this.offset) {
			return this.text.substr(this.offset, pos - this.offset).calc_width2(g_font_12);
		}
		return 0;
	}
	...
	this.GetCPos = function (x) {
		...
		for (var i = this.offset; i < this.text.length; i++) {
			pos += this.text.substr(i, 1).calc_width2(g_font_12);
			if (pos >= tx + 3) {
				break;
			}
		}
