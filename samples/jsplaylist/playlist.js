function oGroup(index, start, metadb, group_key, collapsed) {
	this.index = index;
	this.start = start;
	this.metadb = metadb;
	...
	this.finalise = function (count, total_time_length) {
		...
		if (properties.autocollapse) {
			if (p.list.focusedTrackId >= this.start && p.list.focusedTrackId < this.start + this.count) {
...
function oItem(row_index, type, handle, track_index, group_index, track_index_in_group, heightInRow, groupRowDelta, obj, empty_row_index) {
	...
	this.parseTF = function (tf, default_color) {
		...
		var txt = "", i = 1, tmp = "";
		var pos = tf.indexOf(chars.etx);
		if (pos > -1) {
			var arr = tf.split(chars.etx);
			var fin = arr.length;
	...
	this.drawRowContents = function (gr) {
		...
		if (is_item_selected) {
			gr.FillRectangle(this.x, this.y, this.w, this.h, setAlpha(g_color_selected_bg, 150));
			if (p.list.focusedTrackId == this.track_index) {
				DrawRectangle(gr, this.x + 1, this.y, this.w - 2, this.h - 1, setAlpha(txt_color, 150));
			}

			txt_color = DetermineTextColour(g_color_selected_bg);
			fader_txt = blendColours(txt_color, g_color_selected_bg, 0.25);
		}
	...
	this.draw = function (gr, x, y, w, h) {
		...
		if (this.type == 0) { // track
			...
			if (this.obj && this.heightInRow > 1 && cover.show) {
				...
				DrawCover(gr, this.obj.cover_img, cv_x, cv_y, cv_s, cv_s);
			} else {
				cover_size = g_z4;
			}
			...
			if (this.heightInRow == 1) {
				gr.WriteText(this.l1 + " / " + this.l2, g_font_group1, g_color_normal_txt, this.x + cover_size + text_left_padding, (this.y - groupDelta) - 1, this.w - cover_size - text_left_padding * 4 - lg1_right_field_w - scrollbar_gap, this.h, 0, 2, 1, 1);
				gr.WriteText(this.r1, g_font_group1, g_color_normal_txt, this.x + cover_size + text_left_padding, (this.y - groupDelta) - 1, this.w - cover_size - text_left_padding * 5 + 2 - scrollbar_gap, this.h, 1, 2, 1, 1);
				gr.FillRectangle(this.x + cover_size + text_left_padding, Math.round(this.y + cRow.playlist_h * 1 - groupDelta - 5), this.w - cover_size - text_left_padding * 5 + 2 - scrollbar_gap, 1, g_line_colour);
			} else {
				gr.WriteText(this.l1, g_font_group1, g_color_normal_txt, this.x + cover_size + text_left_padding, (this.y - groupDelta) + 3, this.w - cover_size - text_left_padding * 4 - lg1_right_field_w - scrollbar_gap, cRow.playlist_h, 0, 2, 1, 1);
				gr.WriteText(this.l2, g_font_group2, fader_txt, this.x + cover_size + text_left_padding, (this.y + cRow.playlist_h - groupDelta) - 4, this.w - cover_size - text_left_padding * 4 - lg2_right_field_w - scrollbar_gap, cRow.playlist_h, 0, 2, 1, 1);
				gr.WriteText(this.r1, g_font_group1, g_color_normal_txt, this.x + cover_size + text_left_padding, (this.y - groupDelta) + 3, this.w - cover_size - text_left_padding * 5 + 2 - scrollbar_gap, cRow.playlist_h, 1, 2, 1, 1);
				gr.WriteText(this.r2, g_font_group2, fader_txt, this.x + cover_size + text_left_padding, (this.y + cRow.playlist_h - groupDelta) - 4, this.w - cover_size - text_left_padding * 5 + 1 - scrollbar_gap, cRow.playlist_h, 1, 2, 1, 1);
				gr.FillRectangle(this.x + cover_size + text_left_padding, (this.y + cRow.playlist_h * 2 - groupDelta) - 8, this.w - cover_size - text_left_padding * 5 + 2 - scrollbar_gap, 1, g_line_colour);
				...
				if (this.obj && this.heightInRow > 2) {
					...
					gr.WriteText(lg3_left_field, g_font_12, fader_txt, this.x + cover_size + text_left_padding, (this.y + cRow.playlist_h * 2 - groupDelta) - 4, this.w - cover_size - text_left_padding * 4 - lg3_right_field_w - scrollbar_gap, cRow.playlist_h, 0, 0, 1);
				}
			}
...
function oGroupBy(ref, label, tf, collapsedHeight, expandedHeight, showCover, autoCollapse, collapseByDefault, l1, r1, l2, r2) {
	this.ref = ref;
	this.label = label;
	this.tf = tf;
...
function oList(object_name) {
	this.saveGroupBy = function () {
		var data = ["ref", "label", "tf", "collapsedHeight", "expandedHeight", "showCover", "autoCollapse", "collapseByDefault", "l1", "r1", "l2", "r2"];
	...
	this.initGroupBy = function () {
		...
		var collapsedHeight = window.GetProperty("JSPLAYLIST.GROUPBY2.collapsedHeight" , "2^^2").split("^^");
		...
		var autoCollapse = window.GetProperty("JSPLAYLIST.GROUPBY2.autoCollapse" , "0^^0").split("^^");
		var collapseByDefault = window.GetProperty("JSPLAYLIST.GROUPBY2.collapseByDefault", "0^^0").split("^^");
		...
		for (var i = 0; i < this.totalGroupBy; i++) {
			this.groupby.push(new oGroupBy(ref[i], label[i], tf[i], collapsedHeight[i], expandedHeight[i], showCover[i], autoCollapse[i], collapseByDefault[i], l1[i], r1[i], l2[i], r2[i]));
		}
	}
	...
	this.updateGroupsOnCollapse = function (group_id) {
		if (!this.groups[group_id].collapsed) {
			var delta = this.groups[group_id].count + cGroup.expanded_height - cGroup.collapsed_height;
			var fin = this.groups.length;
	...
	this.updateGroupByPattern = function (pattern_idx) {
		...
		properties.collapseByDefault = this.groupby[pattern_idx].collapseByDefault == "1";
	}
	...
	this.getGroupIdFromRowId = function (valeur) {
		...
		while (deb <= fin) {
			...
			grp_size = this.groups[mediane].collapsed ? grp_height : grp_height + this.groups[mediane].count;
	...
	this.draw = function (gr) {
		...
		if (g_drag_drop_status && g_drag_drop_bottom) {
			...
			gr.FillRectangle(rx, ry + item_height - Math.floor(cList.borderWidth / 2), rw, cList.borderWidth, g_color_selected_bg);
			gr.FillRectangle(rx, ry + item_height - Math.floor(cList.borderWidth / 2) - 4 * cList.borderWidth, cList.borderWidth, 9 * cList.borderWidth, g_color_selected_bg);
