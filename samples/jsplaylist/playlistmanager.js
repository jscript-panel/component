	...
	this.draw = function (gr) {
		...
		if (this.ishoverHeader) {
			if (g_drag_drop_playlist_manager_hover) {
				...
				gr.WriteText("TO A NEW PLAYLIST", g_font_12, blendColours(g_colour_text, g_colour_background, 0.25), cx + g_z5, this.y, cw - g_z10, ch, 0, 2, 1);
			} else {
				gr.FillRectangle(this.x - this.woffset + this.border, this.y, this.w - this.border, ch - 1, g_colour_text & 0x15ffffff);
				gr.WriteText("PLAYLISTS", g_font_12, blendColours(g_colour_text, g_colour_background, 0.25), cx, this.y, cw - g_z5, ch, 0, 2, 1);
			}
		} else {
			if (g_drag_drop_playlist_manager_hover) {
				gr.FillRectangle(this.x - this.woffset + this.border, this.y, this.w - this.border, ch - 1, g_colour_text & 0x15ffffff);
		...
		if (cPlaylistManager.showStatusBar) {
			...
			gr.WriteText(status_txt, g_font_12, blendColours(g_colour_text, g_colour_background, 0.25), fx + 5, fy, fw - 10, fh, 1, 2, 1);
			// draw sort buttons
