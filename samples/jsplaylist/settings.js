...
function settings_radioboxes_action(id, status, pid) {
	...
	case 2:
		...
		if (id >=8 && id <= 12) {
			...
			p.list.groupby[selectedPatternId].expandedHeight = id - 8;
		}

		if (id == 15 || id == 16) {
			p.settings.pages[pid].elements[15].status = id == 15;
			p.settings.pages[pid].elements[16].status = id == 16;
			p.list.groupby[selectedPatternId].collapseByDefault = id == 15 ? 1: 0;
		}
...
function settings_listboxes_action(pageId, id, selectedId) {
	...
	case 2:
		...
		case 0:
			...
			p.settings.pages[2].elements[3].status = tmp == 0;
			p.settings.pages[2].elements[4].status = tmp == 1;
			p.settings.pages[2].elements[5].status = tmp == 2;
			p.settings.pages[2].elements[6].status = tmp == 3;
			p.settings.pages[2].elements[7].status = tmp == 4;

			var tmp = p.list.groupby[selectedId].expandedHeight;
			p.settings.pages[2].elements[8].status = tmp == 0;
			p.settings.pages[2].elements[9].status = tmp == 1;
			p.settings.pages[2].elements[10].status = tmp == 2;
			p.settings.pages[2].elements[11].status = tmp == 3;
			p.settings.pages[2].elements[12].status = tmp == 4;

			var tmp = p.list.groupby[selectedId].collapseByDefault;
			p.settings.pages[2].elements[15].status = tmp == 1;
			p.settings.pages[2].elements[16].status = tmp == 0;

			txtbox_value = p.list.groupby[selectedId].l1;
			p.settings.pages[2].elements[17].inputbox.check("lbtn_down", 0, 0);
...
function oPage(id, objectName, label, nbrows) {
	...
	this.init = function () {
		...
		case 0: // General
			...
			this.elements.push(new oCheckBox(2, 20, cSettings.topBarHeight + rh * 4.25, "Show Playlist Manager (toggle = TAB key)", "cPlaylistManager.visible", this.id));
			this.elements.push(new oCheckBox(3, 20, cSettings.topBarHeight + rh * 5.25, "Show Playlist Scrollbar", "properties.showscrollbar", this.id));
			break;
		case 1: // Columns
		...
		case 2: // Groups
			...
			this.elements.push(new oRadioButton(3, txtbox_x, cSettings.topBarHeight + rh * 9.75, "0", (v == 0), this.id));
			this.elements.push(new oRadioButton(4, txtbox_x + spaceBetween_w, cSettings.topBarHeight + rh * 9.75, "1", (v == 1), this.id));
			this.elements.push(new oRadioButton(5, txtbox_x + spaceBetween_w * 2, cSettings.topBarHeight + rh * 9.75, "2", (v == 2), this.id));
			this.elements.push(new oRadioButton(6, txtbox_x + spaceBetween_w * 3, cSettings.topBarHeight + rh * 9.75, "3", (v == 3), this.id));
			this.elements.push(new oRadioButton(7, txtbox_x + spaceBetween_w * 4, cSettings.topBarHeight + rh * 9.75, "4", (v == 4), this.id));
			...
			this.elements.push(new oRadioButton(8, txtbox_x, cSettings.topBarHeight + rh * 11.75, "0", (v == 0), this.id));
			this.elements.push(new oRadioButton(9, txtbox_x + spaceBetween_w, cSettings.topBarHeight + rh * 11.75, "1", (v == 1), this.id));
			this.elements.push(new oRadioButton(10, txtbox_x + spaceBetween_w * 2, cSettings.topBarHeight + rh * 11.75, "2", (v == 2), this.id));
			this.elements.push(new oRadioButton(11, txtbox_x + spaceBetween_w * 3, cSettings.topBarHeight + rh * 11.75, "3", (v == 3), this.id));
			this.elements.push(new oRadioButton(12, txtbox_x + spaceBetween_w * 4, cSettings.topBarHeight + rh * 11.75, "4", (v == 4), this.id));
			// Create checkbox Cover Art in Group Header ON/OFF
			this.elements.push(new oCheckBox(13, txtbox_x, cSettings.topBarHeight + rh * 13.75, "Enable Cover", "p.list.groupby[p.settings.pages[2].elements[0].selectedId].showCover != 0", this.id));
			// Create checkbox Auto-Collpase ON/OFF
			this.elements.push(new oCheckBox(14, txtbox_x, cSettings.topBarHeight + rh * 15.75, "Enable Auto-Collapse", "p.list.groupby[p.settings.pages[2].elements[0].selectedId].autoCollapse != 0", this.id));

			// Create radio buttons for Default Group Status (Collapsed OR Expanded)
			var spaceBetween_w = scale(90);
			this.elements.push(new oRadioButton(15, txtbox_x, cSettings.topBarHeight + rh * 17.75, "Collapsed", (p.list.groupby[p.settings.pages[2].elements[0].selectedId].collapseByDefault == "1"), this.id));
			this.elements.push(new oRadioButton(16, txtbox_x + spaceBetween_w, cSettings.topBarHeight + rh * 17.75, "Expanded", (p.list.groupby[p.settings.pages[2].elements[0].selectedId].collapseByDefault == "0"), this.id));

			var txtbox_value = p.list.groupby[listBoxCurrentId].l1;
			this.elements.push(new oTextBox(17, txtbox_x, cSettings.topBarHeight + rh * 19, ww - txtbox_x - 20 - this.scrollbarWidth, cHeaderBar.height, "Header line 1, left field", txtbox_value, this.id));
			txtbox_value = p.list.groupby[listBoxCurrentId].r1;
			this.elements.push(new oTextBox(18, txtbox_x, cSettings.topBarHeight + rh * 21, ww - txtbox_x - 20 - this.scrollbarWidth, cHeaderBar.height, "Header line 1, right field", txtbox_value, this.id));
			txtbox_value = p.list.groupby[listBoxCurrentId].l2;
			this.elements.push(new oTextBox(19, txtbox_x, cSettings.topBarHeight + rh * 23, ww - txtbox_x - 20 - this.scrollbarWidth, cHeaderBar.height, "Header line 2, left field", txtbox_value, this.id));
			txtbox_value = p.list.groupby[listBoxCurrentId].r2;
			this.elements.push(new oTextBox(20, txtbox_x, cSettings.topBarHeight + rh * 25, ww - txtbox_x - 20 - this.scrollbarWidth, cHeaderBar.height, "Header line 2, right field", txtbox_value, this.id));
			break;
		case 3: // Appearance
			var rh = cSettings.rowHeight;
	...
	this.draw = function (gr) {
		...
		case 2:
			...
			gr.WriteText("Collapsed Row Height", g_font_12_1, p.settings.color1, txtbox_x, cSettings.topBarHeight + rh * 8.75 - (this.offset * cSettings.rowHeight), p.settings.w - 10, p.settings.txtHeight + 10, 0, 2, 1);
			gr.WriteText("Expanded Row Height", g_font_12_1, p.settings.color1, txtbox_x, cSettings.topBarHeight + rh * 10.75 - (this.offset * cSettings.rowHeight), p.settings.w - 10, p.settings.txtHeight + 10, 0, 2, 1);
			gr.WriteText("Cover Art Status", g_font_12_1, p.settings.color1, txtbox_x, cSettings.topBarHeight + rh * 12.75 - (this.offset * cSettings.rowHeight), p.settings.w - 10, p.settings.txtHeight + 10, 0, 2, 1);
			gr.WriteText("Auto-Collapse Status", g_font_12_1, p.settings.color1, txtbox_x, cSettings.topBarHeight + rh * 14.75 - (this.offset * cSettings.rowHeight), p.settings.w - 10, p.settings.txtHeight + 10, 0, 2, 1);
			gr.WriteText("Default Group Status", g_font_12_1, p.settings.color1, txtbox_x, cSettings.topBarHeight + rh * 16.75 - (this.offset * cSettings.rowHeight), p.settings.w - 10, p.settings.txtHeight + 10, 0, 2, 1);
			break;
		case 3:
...
function oSettings() {
	...
	this.setSize = function (x, y, w, h) {
		...
		if (this.pages.length == 0) {
			...
			this.pages.push(new oPage(2, "p.settings.pages[2]", "Groups", 27));
	...
	this.closeButtonCheck = function (event, x, y) {
		...
		case "lbtn_up":
			if (state == ButtonStates.hover) {
				...
				resize_panels();
				properties.collapseByDefault = p.list.groupby[cGroup.pattern_idx].collapseByDefault != 0;
				update_playlist(properties.collapseByDefault);
