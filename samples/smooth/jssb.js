...
function oBrowser() {
		...
		if (this.list.Count > 0) {
			...
			track_tfs.forEach((function (track_tf, i) {
				...
				if (current != previous) {
					...
					this.groups.push(new oGroup(g + 1, i, handle, meta, cachekey));
		...
		for (var i = g_start_; i < g_end_; i++) {
			...
			case 1:
				...
				if (i == this.selectedIndex) {
					...
					normal_text = DetermineTextColour(g_color_selected_bg);
					fader_txt = blendColours(normal_text, g_color_selected_bg, 0.25);
				}
	...
	this.context_menu = function (x, y, albumIndex) {
		...
		var menu1 = window.CreatePopupMenu();
		...
		menu1.AppendMenuItem(MF_STRING, 1000, "Send to default playlist and play");
		menu1.AppendMenuItem(MF_STRING, 1001, "Send to default playlist");
		menu1.CheckMenuRadioItem(1000, 1001, ppt.sendto_playlist_play ? 1000 : 1001);
		menu1.AppendMenuSeparator();
		menu1.AppendMenuItem(MF_STRING, 1002, "Default playlist name")
		menu1.AppendTo(menu, MF_STRING, "Double click");
	...
	this.settings_context_menu = function (x, y) {
		...
		menu1.AppendMenuItem(CheckMenuIf(ppt.enableCustomColours), 2, "Enable");
		menu1.AppendMenuSeparator();
		menu1.AppendMenuItem(colour_flag, 3, "Text");
		menu1.AppendMenuItem(colour_flag, 4, "Background");
		menu1.AppendMenuItem(colour_flag, 5, "Selected background");
		menu1.AppendTo(menu, MF_STRING, "Custom colours");
		menu.AppendMenuSeparator();
